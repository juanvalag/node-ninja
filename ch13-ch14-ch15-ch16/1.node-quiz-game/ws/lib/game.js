...
export async function GameFactory(gameId) {
    ...
    if (!gamesActive.has(gameId)) {

        // create new game instance
        const game = new Game();
        if (await game.create(gameId)) {
            gamesActive.set(gameId, game);
        }

        console.log(`Game ${gameId} added - active games on this server: ${gamesActive.size}`);

    }

    return gamesActive.get(gameId) || null;
...
async function gameComplete(gameId) {
    ...
    if (!gamesActive.has(gameId)) return;

    await db.gameRemove(gameId);
    gamesActive.delete(gameId);

    console.log(`Game ${gameId} removed - active games on this server: ${gamesActive.size}`);
...
class Game {
    ...
    #clientSend(type, data) {
        this.players.forEach(p => p.send(type, data))
    }
        ...
        switch (type) {
            ...
            case 'questionanswered':
                ...
                if (!this.#state.playersAnswered) {
                    ...
                    // first response?
                    if (!this.#state.playersAnswered && this.players.size > 1) {
    ...
    async #eventHandler({ gameId, type, data }) {
        ...
        switch (type) {
            ...
            case 'questionactive':
                ...
                this.players.forEach(p => p.scoreQuestion = noAnswer);
                break;
            ...
            case 'scoreboard':
                ...
                this.#state.current = type;
                this.players.forEach(p => p.scoreTotal += p.scoreQuestion); // calculate scores
                data = this.playerAll();
                break;
    ...
    playerAll() {
        return Array.from(this.players, ([i, p]) => { return { id: p.id, name: p.name, score: p.scoreTotal } });
    }
